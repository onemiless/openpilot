...
def test_tesla_cruise_system_log_includes_recent_panda_tx_rejection(monkeypatch):
  panda = SimpleNamespace(
    ...
    heartbeatLost=False, faultStatus=SimpleNamespace(raw=0), faults=[],
    uptime=120,
    rxBufferOverflow=0, txBufferOverflow=0, safetyModel=SimpleNamespace(raw=10), safetyParam=0,
    canState0=_can_state(), canState1=_can_state(transmitErrorCnt=0), canState2=_can_state(),
  )
  ...
  daemon.sm = FakeSubMaster({
    "pandaStates": [panda],
    "radarState": SimpleNamespace(radarErrors=radar_errors),
