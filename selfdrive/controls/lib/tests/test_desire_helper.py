...
def test_automatic_overtake_blinker_is_latched_through_lane_change():
  ...
  intent = update_automatic_blinker_intent(
    intent, LaneChangeState.laneChangeStarting, LaneChangeDirection.left, BLINKER_NONE, BLINKER_NONE,
  )
  assert intent == BLINKER_LEFT

  intent = update_automatic_blinker_intent(
    intent, LaneChangeState.laneChangeFinishing, LaneChangeDirection.left, BLINKER_NONE, BLINKER_NONE,
  )
  assert intent == BLINKER_LEFT
