...
MPC_TUNING_PARAMS = {
  ...
  "MpcTFollowAggressive": ("t_follow_aggressive", 50, 400),
}


def _read_scaled_param(params, key, default, min_value, max_value):
  try:
    raw = int(params.get(key, return_default=True))
  except (TypeError, ValueError):
    raw = int(default * MPC_TUNING_SCALE)
  clipped = max(min_value, min(max_value, raw))
  if clipped != raw:
    params.put(key, clipped, block=True)
  return clipped / MPC_TUNING_SCALE
...
def read_long_mpc_tuning(params=None):
  ...
  for key, (field, min_value, max_value) in MPC_TUNING_PARAMS.items():
    values[field] = _read_scaled_param(params, key, getattr(DEFAULT_LONG_MPC_TUNING, field), min_value, max_value)
