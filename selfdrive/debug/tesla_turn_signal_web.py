...
<head>
  ...
  <style>
    ...
    .drive-grid { display:grid; grid-template-columns:repeat(2,minmax(0,1fr)); gap:10px; } .metric { background:#1e293b; border-radius:13px; padding:14px; } .metric span { display:block; color:#94a3b8; font-size:13px; } .metric strong { display:block; margin-top:5px; font-size:22px; } .drive-alert { white-space:pre-wrap; } #driving-canvas { display:block; width:100%; height:360px; margin:12px 0; border-radius:13px; background:linear-gradient(#172554,#020617); }
  </style>
</head><body><main>
  ...
  <section id="driving-panel" hidden><h1>行驶信息</h1><p>只读实时仪表盘，每秒最多更新两次；轨迹由手机绘制，不启动视频或屏幕采集。</p><div id="driving-state" class="notice">正在连接车辆数据…</div><canvas id="driving-canvas" aria-label="预测道路轨迹"></canvas><div id="driving-info" class="drive-grid"></div><div id="driving-alert" class="notice drive-alert" hidden></div></section>
...
function drawDrivingGeometry(geometry) { const canvas = document.getElementById('driving-canvas'), ratio = window.devicePixelRatio || 1, width = Math.max(1, canvas.clientWidth), height = Math.max(1, canvas.clientHeight); if (canvas.width !== width * ratio || canvas.height !== height * ratio) { canvas.width = width * ratio; canvas.height = height * ratio; } const ctx = canvas.getContext('2d'); ctx.setTransform(ratio, 0, 0, ratio, 0, 0); ctx.clearRect(0, 0, width, height); ctx.fillStyle = '#020617'; ctx.fillRect(0, 0, width, height); ctx.setTransform(ratio, 0, 0, ratio, 0, 0); const xScale = (height - 48) / 100, yScale = Math.min(width / 10, 30); ctx.setLineDash([7,7]); (geometry.edges || []).forEach(line => drawLine(ctx, line, xScale, yScale, '#64748b', 2)); ctx.setLineDash([]); (geometry.lanes || []).forEach(line => drawLine(ctx, line, xScale, yScale, '#e2e8f0', 2)); drawLine(ctx, geometry.path || [], xScale, yScale, '#22c55e', 5); ctx.fillStyle = '#2563eb'; ctx.fillRect(width / 2 - 14, height - 30, 28, 20); (geometry.leads || []).forEach(lead => { const px = width / 2 + lead.y * yScale, py = height - 32 - lead.x * xScale; if (py > 0 && py < height) { ctx.fillStyle = '#f97316'; ctx.fillRect(px - 7, py - 7, 14, 14); } }); }
async function loadDrivingStatus() { if (drivingLoading) return; drivingLoading = true; const state = document.getElementById('driving-state'), root = document.getElementById('driving-info'), alert = document.getElementById('driving-alert'); try { const r = await fetch('/api/driving-status', {cache:'no-store'}); const data = await r.json(); if (!r.ok) throw new Error(data.message || 'HTTP ' + r.status); const connected = Object.values(data.connected).every(Boolean); state.textContent = !data.onroad ? '设置模式：等待车辆启动。' : connected ? '行驶中：车辆数据正常。' : '行驶中：部分车辆数据暂未收到。'; state.className = 'notice' + ((!data.onroad || !connected) ? ' onroad' : ''); drawDrivingGeometry(data.geometry); root.replaceChildren(metric('当前车速', data.speed_kph.toFixed(1) + ' km/h'), metric('巡航设定', data.set_speed_kph.toFixed(1) + ' km/h'), metric('加速度', data.acceleration.toFixed(2) + ' m/s²'), metric('方向盘角度', data.steering_angle_deg.toFixed(1) + '°'), metric('辅助驾驶', boolText(data.openpilot_enabled, '已接管', '未接管')), metric('MADS 横向', boolText(data.mads_enabled, '已启用', '未启用')), metric('踏板 / 静止', (data.gas_pressed ? '油门 ' : '') + (data.brake_pressed ? '刹车 ' : '') + (data.standstill ? '静止' : '行驶') || '无'), metric('设备', data.temperature_c === null ? data.power_draw_w.toFixed(1) + ' W' : data.power_draw_w.toFixed(1) + ' W / ' + data.temperature_c.toFixed(0) + '°C')); alert.hidden = !data.alert; alert.textContent = data.alert || ''; } catch (e) { state.textContent = '行驶数据读取失败：' + e; state.className = 'notice onroad'; } finally { drivingLoading = false; } }
