...
CarrotPanel::CarrotPanel(QWidget* parent) : QWidget(parent) {
  ...
  speedToggles->addItem(new CValueControl("TeslaSpeedSyncEnabled", "Tesla 原车巡航速度同步(1)", "读取Tesla车机融合地图限速并加上道路限速偏移，通过右滚轮每次+1/-1同步到原车巡航。手动拨轮暂停；1秒内上下或下上连续拨动，或关闭再开启CP后恢复。修改开关后需重启车辆", 0, 1, 1));
