...
CarrotPanel::CarrotPanel(QWidget* parent) : QWidget(parent) {
  ...
  speedToggles->addItem(new CValueControl("TeslaSpeedSyncEnabled", "Tesla 原车巡航速度同步(1)", "每次开启CP时锁定读取Tesla融合地图限速和道路限速偏移，通过右滚轮每次+1/-1同步；每秒一步，每5步暂停5秒。手动拨轮暂停；1秒内上下或下上连续拨动，或关闭再开启CP后恢复。修改开关后需重启车辆", 0, 1, 1));
