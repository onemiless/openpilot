#!/usr/bin/env python3
"""Offroad-only official UFLDv1 TuSimple benchmark on the selected tinygrad device."""
...
from openpilot.sunnypilot.lane_topology.ufldv1_adapter import UFLDv1OnnxLaneModel
...
def main() -> int:
  ...
  if not params.get_bool("IsOffroad"):
    raise RuntimeError("UFLDv1 lane benchmark requires IsOffroad=1")
  ...
  model = UFLDv1OnnxLaneModel(args.model)
  load_seconds = time.perf_counter() - load_started

  warmup_latencies = []
  for _ in range(3):
    started = time.perf_counter()
    logits = model.logits(rgb)
    warmup_latencies.append((time.perf_counter() - started) * 1000.0)

  latencies = []
  for run in range(args.runs):
    if run % 20 == 0 and not params.get_bool("IsOffroad"):
      raise RuntimeError("IsOffroad changed during UFLDv1 benchmark")
    started = time.perf_counter()
    logits = model.logits(rgb)
    latencies.append((time.perf_counter() - started) * 1000.0)
    if logits.shape != (1, 101, 56, 4) or not np.all(np.isfinite(logits)):
      raise RuntimeError(f"invalid UFLDv1 output at run {run + 1}: {logits.shape}")

  model.close()
  if trace["bulk_fail"] or trace["pending"] or trace["f2_in"] != trace["bulk_ok"]:
    raise RuntimeError(f"incomplete USB trace: {trace}")
  report = {
    "schema": "ufldv1-tusimple-res18-usbgpu-benchmark-v1",
    "status": "PASS",
    "model": str(args.model),
    "model_sha256": model.model_sha256,
    "input_shape": [1, 3, 288, 800],
    "output_shape": [1, 101, 56, 4],
