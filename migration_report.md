...
- `controlsd` 按来源恢复 `CP.steerAtStandstill` 语义；Tesla 已声明该能力，因此在 D 挡原地且 MADS/openpilot 横向已取得控制权限、转向系统无故障时可执行静态点灯测试。原地不会产生变道完成状态，需要网页取消或等待 session 超时。
...
- 按用户当前测试要求，8090 暂无认证。服务没有 CORS 通配、没有通用 Params 写接口，限制 4 KiB 请求体和单客户端每秒 5 次动作请求。同类动作只允许一个待消费 Params 请求，重复提交返回 409，避免异步写覆盖。网页显著提示测试结束后关闭参数并重启。
...
- 任意手动非零拨轮动作暂停自动同步。上下或下上在不超过 1 秒内连续出现时恢复；超过 1 秒继续保持手动暂停。按来源行为，纵向/巡航退出后重新接合或有效道路限速目标改变时也会清除手动暂停。
- 未处于自身 tick 反馈等待期间的 Tesla 巡航设定速度外部变化会进入手动暂停，防止控制器反向追赶驾驶员或 OEM 的新设定值。
