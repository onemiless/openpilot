...
from openpilot.common.hardware import HARDWARE, COMMA_HARDWARE, AGNOS
...
class Chestnut:
  # Flash only while offroad. modeld ignores Chestnut until the product string
  # confirms the bundled firmware version.
  MAX_ATTEMPTS = 3
  RETRY_INTERVAL = 20.
...
def hw_state_thread(end_event, hw_queue):
  ...
  while not end_event.is_set():
    ...
    # These are expensive calls. Update every 10s or immediately after a USB
    # topology change so Chestnut attach/re-enumeration is visible promptly.
...
def hardware_thread(end_event, hw_queue) -> None:
  ...
  while not end_event.is_set():
    ...
    # Automatic shutdown is temporarily disabled to avoid unexpected device reboots.
