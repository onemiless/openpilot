...
# openpilot 基础模块
# 注意：新版 sunnypilot 顶层包根在 git 根下 openpilot/ 子目录，import 写法 openpilot.xxx
# （不要写成 openpilot.openpilot.xxx）
# system.hardware 版本易变，HARDWARE 在函数内 try-import 兜底；仅 Params 为必需项
...
def get_serial():
  """获取设备序列号。优先走硬件抽象；失败/新版结构异常时 fallback 主机名并去掉 comma- 前缀。"""
  try:
    import openpilot.system.hardware as _hw
    return _hw.HARDWARE.get_serial()
  except Exception:
    # fallback：nodename 为 "comma-XXXX"，去掉前缀只留纯 serial，保证前后端一致
    return os.uname().nodename.replace("comma-", "", 1)
...
def get_device_type():
  try:
    import openpilot.system.hardware as _hw
    if hasattr(_hw.HARDWARE, 'get_device_type'):
      dt = _hw.HARDWARE.get_device_type()
...
async def execute_cmd(command, timeout=30):
  ...
  try:
    ...
    return {"status": "error", "output": f"命令执行超时({timeout}s)，进程已终止"}
  except Exception as e:
    _kill_process_group(proc)
