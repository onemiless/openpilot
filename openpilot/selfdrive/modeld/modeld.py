...
class ChestnutState:
  # modeld owns the Chestnut GPU, so telemetry is collected here rather than
  # from hardwared.
  ...
  def send(self) -> None:
    ...
    if "AMD" in Device._opened_devices:
      try:
        # ASM runs on USB-C power, so these reads remain available without a GPU.
  ...
  while True:
    ...
    if sm.updated["extrinsicsCalibration"] and sm.seen['narrowRoadCameraState'] and sm.seen['deviceState']:
      ...
      model_transform_main, model_transform_extra = camera_offset_helper.update(
        model_transform_main, model_transform_extra, sm, main_wide_camera, has_wide_camera,
      )
