...
def test_registry_has_stable_order_and_fallback():
  ...
  assert get_backend(0).id == BackendId.SP_UPSTREAM_TUNABLE
  assert get_backend(1).id == BackendId.LOCAL
  assert get_backend(2).id == BackendId.TN_NO_DEC
  assert get_backend(None).id == BackendId.SP_UPSTREAM_TUNABLE
  assert get_backend("bad").id == BackendId.SP_UPSTREAM_TUNABLE
  validate_registry()
...
def test_legacy_defaults_migrate_without_numeric_drift():
  ...
  local = resolve_tuning(snapshot, BACKENDS[BackendId.LOCAL]).native_values
  assert local["x_ego_obstacle_cost"] == MPC_OFFICIAL_VALUES["MpcXObstacleCost"] / 100.0
  assert local["comfort_brake"] == MPC_OFFICIAL_VALUES["MpcComfortBrake"] / 100.0
  assert local["stop_distance"] == MPC_OFFICIAL_VALUES["MpcStopDistance"] / 100.0
  assert local["t_follow_standard"] == MPC_OFFICIAL_VALUES["MpcTFollowStandard"] / 100.0
...
def test_one_shot_migration_preserves_legacy_sp_solver_fixed_values():
  ...
  sp = resolve_tuning(snapshot, BACKENDS[BackendId.SP_UPSTREAM_TUNABLE]).native_values
  assert sp["comfort_brake"] == 2.5
  assert sp["stop_distance"] == 6.0
