...
from openpilot.selfdrive.controls.lib.longitudinal_backends.tn_no_dec.accel_controller.constants import (
...
from openpilot.selfdrive.controls.lib.longitudinal_backends.tn_no_dec.accel_controller.helpers import build_accel_ceiling, is_valid_context
from openpilot.selfdrive.controls.lib.longitudinal_backends.tn_no_dec.accel_controller.lead import calculate_lead_plan
from openpilot.selfdrive.controls.lib.longitudinal_backends.tn_no_dec.accel_controller.pace import Pace
from openpilot.selfdrive.controls.lib.longitudinal_mpc_lib.long_mpc_tn import LongitudinalPlanSource
from openpilot.selfdrive.controls.lib.longitudinal_backends.registry import BACKENDS, BackendId
from openpilot.selfdrive.controls.lib.longitudinal_backends.tuning import load_resolved_tuning
...
class AccelController:
  ...
  def update_params(self) -> None:
    if self._param_frame % self._param_read_frames == 0:
      try:
        resolved = load_resolved_tuning(self.params, BACKENDS[BackendId.TN_NO_DEC])
      except ValueError:
        resolved = None
      if resolved is not None:
        self.enabled = bool(resolved.native_values["accel_personality_enabled"])
        self.profile = sanitize_profile(int(resolved.native_values["accel_personality_profile"]))
      else:
