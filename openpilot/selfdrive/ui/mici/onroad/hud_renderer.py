...
class HudRenderer(Widget):
  ...
  def _render(self, rect: rl.Rectangle) -> None:
    ...
    if egpu_icon_visible(connected=resolve_egpu_connection(ui_state.sm['deviceState'])):
  ...
  def _draw_model_source(self, rect: rl.Rectangle) -> None:
    ...
    alpha = self._egpu_alpha_filter.update(egpu_icon_visible(connected=resolve_egpu_connection(ui_state.sm['deviceState'])))
    if alpha < 1e-2:
      return
