...
from openpilot.sunnypilot.models.helpers import bundle_requires_usbgpu, get_active_source, get_selected_bundle, resolve_bundle_by_ref
from openpilot.sunnypilot.models.model_name import DEFAULT_BIG_MODEL, DEFAULT_MODEL


def active_source() -> str:
  return get_active_source(ui_state.params)


def bundles_for_source(source: str):
  bundles = [bundle for bundle in ui_state.sm["modelManagerSP"].availableBundles
             if bundle_requires_usbgpu(bundle) == (source == "usbgpu")]
  return bundles or get_cached_bundles(ui_state.params, source)
...
def big_model_state() -> str | None:
  ...
  if ui_state.started and active_source() == "usbgpu" and not ui_state.usbgpu:
    return 'failed'
...
def carrying_model() -> tuple[str | None, str | None, str | None]:
  ...
  if source == "usbgpu":
    if ui_state.started and (not ui_state.usbgpu or ui_state.big_model_failed):
