...
const canText = {
  ...
  camera_detection:'相机区域检测', positioned_object:'CH 定位对象', collision_warning:'行人碰撞告警',
...
function drawCanvasSummary(ctx, can, width) {
  const vehicles=can.vehicles||[], pedestrian=can.pedestrian_detection||{}, positionedPedestrians=vehicles.filter(v=>v.type==='pedestrian').length, chips=[];
  ...
  if(pedestrian.collision_warning)chips.push(['行人碰撞告警','#ef4444']);
