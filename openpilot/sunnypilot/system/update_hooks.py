...
def hydrate_lfs_checkout(cwd: str | Path, run: CommandRunner) -> None:
  ...
  if missing:
    # checkout never downloads missing objects. A fresh install or an updater
    # inheriting GIT_LFS_SKIP_SMUDGE otherwise fails every finalization forever.
    run(["git", "lfs", "pull", "--include=" + ",".join(missing), "--exclude=", "origin"], cwd)
    run(["git", "lfs", "checkout"], cwd)
    lfs_files = run(["git", "lfs", "ls-files"], cwd)
    missing = [line.split(" - ", 1)[1] for line in lfs_files.splitlines() if re.fullmatch(r"[0-9a-f]+ - .+", line)]
  if missing:
