...
def test_c3xl_uses_gpio_alerts_without_portaudio_processes(monkeypatch):
  params = Mock()
  ...
  assert not process_config.managed_processes["soundd"].should_run(True, params, CP)
  assert process_config.managed_processes["alert_output"].should_run(True, params, CP)
