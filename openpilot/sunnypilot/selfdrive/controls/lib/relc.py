...
class RoadEdgeLaneChangeController:
  def __init__(self, desire_helper=None):
    # Keep the optional argument for compatibility with this fork's older RELC
    # callers. Lane-change state remains owned by DesireHelper.
  ...
  def set_enabled(self, enabled: bool) -> None:
    """Compatibility helper for local tests and callers of the older RELC API."""
  ...
  def update(self, road_edge_stds, lane_line_probs, v_ego: float, road_edges=None) -> None:
    ...
    if road_edges is not None and len(road_edges) == 2 and len(road_edges[0].y) > 0 and len(road_edges[1].y) > 0:
      ...
      # Missing geometry is treated conservatively once the model confidence
      # conditions are met. Normal modelV2 messages always include roadEdges.
      left_clearance = 0.0
      right_clearance = 0.0
  ...
  def update_and_fill(self, modelv2, mdv2sp, v_ego: float):
  ...
  def is_lane_change_blocked(self, direction):
    ...
    if direction == log.LaneChangeDirection.right:
