...
void Localizer::handle_extrinsics_calibration(double current_time, const cereal::ExtrinsicsCalibration::Reader& log) {
  if (!this->is_timestamp_valid(current_time)) {
    this->observation_timings_invalid = true;
    return;
  }
...
void Localizer::handle_msg(const cereal::Event::Reader& log) {
  ...
  //  this->handle_gnss(t, log.getGnssMeasurements());
    ...
    this->handle_extrinsics_calibration(t, log.getExtrinsicsCalibration());
  }
  this->finite_check();
