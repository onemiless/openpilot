sunnypilot Version 2026.002.006 (2026-08-08)
========================
* Tesla MADS 混控安全回退：
  * 因实车出现高速方向盘剧烈摆动，将驾驶员转向接管、停车暂停/恢复和纵横向事件处理完整恢复至 2026-08-06 的稳定实现。
  * 恢复 Panda 在驾驶员强制转向时的原有安全退出语义，不再让新增混控状态绕过 `steerDisengage`。
* C3XL/Tesla 离线唤醒防循环：
  * 修复休眠车辆持续发送 `VCFRONT_vehiclePowerState=1` 时被误判为启动，造成关机后立即反复唤醒的问题。
  * 电源状态仅在 Tesla 明确进入 `DRIVE=3` 时触发；前门、门把手和通用 CAN 后备唤醒保持不变。

sunnypilot Version 2026.002.005 (2026-08-08)
========================
* Tesla 低速 MADS 混控交接修复：
  * 低速人工转向达到 0.5 Nm 时立即让权；退出使用 0.25 Nm 迟滞阈值，避免扭矩在临界点波动时反复启停 EPS 控制。
  * 仅在方向盘转速低于 10°/s 且稳定松手 0.2 秒后平滑恢复模型控制；重新施力会立即清零恢复计时。
  * 保持低速松手后可恢复模型，不重新引入依赖车速升高才能退出的长期锁存。

sunnypilot Version 2026.002.004 (2026-08-08)
========================
* 蜂鸣器实机调校：
  * 根据设备现场分档试听结果，将所有 MADS 蜂鸣脉冲统一调整为 10 毫秒，兼顾可辨识度和较低音量。

sunnypilot Version 2026.002.003 (2026-08-08)
========================
* Tesla MADS 混控修复：
  * 取消低速/倒挡停车横向控制的持续锁存；仅在低速且驾驶员实际转动方向盘期间暂停输出，松手后立即由现有平滑限速器恢复模型控制。
  * 恢复纵向状态机先处理启用、刹车和取消事件，MADS 再独立处理横向状态，修复退出纵向后无法再次开启以及刹车偶发影响保持模式的问题。
  * MADS 蜂鸣脉冲由 0.2 毫秒增加到 0.5 毫秒，为现有时长的 2.5 倍。
...
========================
* C3XL/Tesla 稳定性修复：
  * 修复倒车切换前进后，低速大角度人工转向可能触发混控左右摆动；停车人工接管期间暂停横向输出，并在车速和驾驶员释放条件满足后平滑恢复。
  * 修复 MADS-only 状态下巡航启用事件先于 MADS 过滤，可能意外恢复纵向控制的问题。
  * 离线监控启动时重新确认全部物理 CAN 收发器开启；布防蓝灯改为每 4 秒短闪，CAN 活动保持快闪，唤醒请求保持常亮。
  * 蜂鸣器改为直接写 GPIO，提示脉冲缩短至原来的 1/5，并串行化重叠提示。
  * 启用有容量上限、仅状态变化写入的 Tesla MADS 故障记录。

sunnypilot Version 2026.002.000 (2026-08-02)
========================
* 此版本仅限C3XL使用
* 修改说明：
