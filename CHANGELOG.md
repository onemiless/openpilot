sunnypilot dev（2026-08-01）
========================
* 基础说明：基于羊大佬 4 月份的 C3XL 分支更新为基础，并增加以下功能（仅测试用）：
* 2026 年 4 月 16 日以来的主要更新：
  * 同步 sunnypilot/openpilot 的系统、界面、模型、硬件兼容性与稳定性修复，并选择性同步羊大佬分支后续的系统更新和硬件 Bug 修复。
  * Tesla 动态 ACC：可按高、低速阈值在 sunnypilot 纵向与原厂 ACC 间切换，支持打转向灯或进入弯道时按开关切回 sunnypilot，并优化双向接管的加速度衔接。
  * Tesla AP 混控与动态 AP：低速由 sunnypilot 横纵向控制，高速由原厂 AP 横纵向控制；驾驶员转向干预或打转向灯时，sunnypilot 横向可及时介入。
  * 完善 AP/ACC/SCC-V 控制源显示：AP 纵向使用黄色 AP，AP 横纵向均由原厂控制时使用蓝色 AP。
  * 修复 AP 或 SP 状态下踩刹车、AP 退出、动态控制源切换时可能出现的 MADS 横向退出、LKAS 设置无效、controls mismatch 等问题。
  * 新增 Tesla 自动设定速度：根据车机地图限速及“巡航－速度限制”偏移自动发送滚轮调速；支持手动覆盖、上下或下上滚轮快捷恢复，以及达到设定速度后停止偏移。
  * 新增 Tesla 车道中心偏移、停止线额外减速度和纵向 MPC 参数/预设调校入口。
  * 完善 Tesla 离线关机与 Panda CAN 唤醒链路，补充点火、车门唤醒、SoM bootkick 重试及断电恢复处理。
  * 增加可选启动目录、阿里云 NTP 时间同步、界面与中文翻译调整，并保留 4 月 16 日以后同步的系统更新和硬件修复。
  * 新增本地网页设置页：可通过设备 IP 的 8088 端口在手机或电脑上按设备菜单分类查看、修改现有白名单设置；行驶中仅开放可实时调整的项目。
  * 保留原有 Tesla 左、右转向 CAN 测试入口，并将 Tesla MADS 屏幕控制改为直观的开关按钮。
  * 新增网页终端：仅在设置模式且设备端启用时可用，默认密码为 `123456`，可在网页中自行修改；命令最长执行 20 秒，输出上限为 64 KiB。
  * 网页设置新增只读“行驶信息”页，实时显示车辆速度、巡航设定、辅助驾驶/MADS 状态、转向与踏板状态、告警及设备电量/温度。
  * 行驶信息页增加手机端 Canvas 轨迹仪表盘：显示模型预测路径、当前车道线、道路边缘与前车位置；仅传输降采样模型坐标，不采集屏幕或启动视频编码。
  * Canvas 增加只读 Tesla 原车交通控制叠加：显示已验证 CAN 报文中的红绿灯颜色与停止线距离；数据超时自动隐藏，且绝不参与横向或纵向控制。
  * 修复 Tesla 首次启动时安全固件同步可能误报“控制指令不一致”；P 档时界面自动返回主页/设置页，不再持续显示道路摄像头。
  * 修复网页左右转测试因旧 Panda 安全固件阻断而导致 SP 退出的问题；Panda 固件版本现随 opendbc 安全规则更新自动刷新。
  * 网页“设备”分类新增 Wi-Fi 热点开关；热点开启后连接设备热点并访问 `http://192.168.43.1:8088` 即可进入设置页面。
  * Tesla 转向 CAN 测试默认启用，并可在“开发者”分类中按需关闭；该测试仍只能在设置模式发起。
* 发布清理：移除仅用于开发验证的 CAN/唤醒测试入口与 MADS 几指选择；保留诊断代码但默认停止写入专项日志，便于后续按需恢复排查。
