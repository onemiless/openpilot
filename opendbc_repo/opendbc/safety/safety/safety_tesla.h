...
static void tesla_rx_hook(const CANPacket_t *to_push) {
  ...
  if (bus == 2) {
    if (tesla_longitudinal && (addr == 0x2b9)) {
      ...
      // Complete an explicit handoff only on the exact counter following the
      // last accepted CP cancel. Since RX state is updated before forwarding,
      // this same validated OEM frame becomes the first restored frame.
...
static bool tesla_tx_hook(const CANPacket_t *to_send) {
  ...
  if (addr == 0x2b9) {
    ...
    // AEB event 3 is an internal ownership-release marker. Consume it in
    // safety so it can atomically restore OEM 0x2B9 forwarding without ever
    // reaching the vehicle bus.
    ...
    if (tesla_longitudinal) {
      ...
      const bool cp_control = !tesla_stock_longitudinal_active && !tesla_longitudinal_handoff_pending && controls_allowed && (acc_state == 4);
