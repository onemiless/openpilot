...
def setup_interfaces(CI, CP: structs.CarParams, CP_SP: structs.CarParamsSP,
  ...
  _initialize_volkswagen(CP, CP_SP, params_dict)
...
def _initialize_volkswagen(CP: structs.CarParams, CP_SP: structs.CarParamsSP, params_dict: dict[str, str]) -> None:
  if CP.brand == 'volkswagen':
    avoid_eps_lockout = int(params_dict.get("VagAvoidEPSLockout", 0)) == 1
    if avoid_eps_lockout:
      CP_SP.flags |= VolkswagenFlagsSP.AVOID_EPS_LOCKOUT.value
