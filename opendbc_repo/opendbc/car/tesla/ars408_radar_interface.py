...
def object_rejection_reason(obj, previously_tracked=False, timed_out=False, max_distance=ARS408_MAX_DISTANCE):
  ...
  # DynProp can classify distant vehicles as stationary/candidate/crossing
  # stationary. Apply the common confidence hysteresis above, and retain the
  # lateral corridor to reject obvious roadside infrastructure.
...
def objects_represent_same_target(first, second, handover=False, first_class=None, second_class=None):
  ...
  first_class = int(first.get("Obj_Class", 7)) if isinstance(first, dict) else int(first_class if first_class is not None else 7)
  second_class = int(second.get("Obj_Class", 7)) if isinstance(second, dict) else int(second_class if second_class is not None else 7)
...
class ARS408RadarInterface(RadarInterfaceBase):
  def __init__(self, CP):
    ...
    self.radar_state_seq = self.params.get_int("TeslaRadarStateSeq")
    self.filter_state_seq = self.params.get_int("TeslaRadarFilterStateSeq")
  ...
  def _update_radar_state(self, timestamp, data, src):
    ...
    self.params.put_int_nonblocking("TeslaRadarStateSeq", self.radar_state_seq)
    self.params.put_int_nonblocking("TeslaRadarStateUpdatedAt", int(time.monotonic()))
  ...
  def _apply_runtime_configuration(self, state):
      ...
      if max_distance < getattr(self, "runtime_max_distance", ARS408_MAX_DISTANCE):
        for object_id, point in list(self.pts.items()):
          if math.hypot(float(point.dRel), float(point.yRel)) > max_distance:
            ...
            self.object_classes.pop(object_id, None)
    ...
    if not extended_enabled and getattr(self, "runtime_extended_enabled", True):
      for object_id, point in self.pts.items():
        point.aRel = 0.0
        self.object_classes[object_id] = 7
    ...
    if snapshot != getattr(self, "last_published_radar_config", None):
      ...
      for key, value in zip(keys, snapshot, strict=True):
        self.params.put_int_nonblocking(key, int(value))
  ...
  def _update_filter_state(self, timestamp, address, data, src):
    ...
    self.params.put_int_nonblocking("TeslaRadarFilterStateSeq", self.filter_state_seq)
  ...
  def _ensure_tracking_state(self):
    if not hasattr(self, "object_classes"):
      self.object_classes = {}
  ...
  def _handoff_track(self, old_raw_id, new_raw_id, reason, old_obj, new_obj):
    ...
    old_class = self.object_classes.pop(old_raw_id, 7)
    logical_id = self.raw_to_track_id.pop(old_raw_id, int(point.trackId))
    self.pts[new_raw_id] = point
    self.object_classes[new_raw_id] = int(new_obj.get("Obj_Class", old_class)) if isinstance(new_obj, dict) else old_class
    ...
    for new_raw_id in [raw_id for raw_id in active_cycle_ids if raw_id not in self.pts]:
      candidates = [old_raw_id for old_raw_id in missing_tracked_ids if old_raw_id in self.pts and
                    objects_represent_same_target(self.pts[old_raw_id], cycle_objects[new_raw_id],
                                                   first_class=self.object_classes.get(old_raw_id, 7))]
      if candidates:
  ...
  def _build_result(self, timestamp):
    ...
    for object_id, obj in cycle_objects.items():
      ...
      # Keep classification internal: stock RadarPoint has no ARS408 class field.
      self.object_classes[object_id] = int(obj.get("Obj_Class", 7))
