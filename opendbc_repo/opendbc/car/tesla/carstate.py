...
class CarState(CarStateBase):
  ...
  def update(self, can_parsers) -> structs.CarState:
    ...
    cruise_state = self.can_define.dv["DI_state"]["DI_cruiseState"].get(cruise_state_raw, None)
    ...
    # Preserve the raw Tesla-reported cruise fault fields. These are evidence
    # from the vehicle, while the controller timing fields added later are only
    # observations correlated with the transition.
    ...
    self.cruise_diagnostics = {
      ...
      "cruise_state_raw": cruise_state_raw,
      ...
      "autopilot_state_raw": autopilot_state,
    ...
    self.cruise_diagnostics.update({
      "oem_2b9_state": int(self.das_control["DAS_accState"]),
      "oem_2b9_state_name": self.can_define.dv["DAS_control"]["DAS_accState"].get(
        int(self.das_control["DAS_accState"]), f"UNKNOWN_{int(self.das_control['DAS_accState'])}"),
      "oem_2b9_aeb_event": int(self.das_control["DAS_aebEvent"]),
      "oem_2b9_aeb_event_name": self.can_define.dv["DAS_control"]["DAS_aebEvent"].get(
        int(self.das_control["DAS_aebEvent"]), f"UNKNOWN_{int(self.das_control['DAS_aebEvent'])}"),
      ...
      "das_status2_nanos": int(cp_ap_party.ts_nanos["DAS_status2"]["DAS_status2Counter"]),
    })
