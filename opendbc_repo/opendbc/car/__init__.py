...
class AngleSteeringLimitsVM:
  """Vehicle-model steering limits used by current Tesla/openpilot."""
  STEER_ANGLE_MAX: float
  MAX_LATERAL_ACCEL: float = 3.0 + (ACCELERATION_DUE_TO_GRAVITY * 0.06)
  MAX_LATERAL_JERK: float = 3.0 + (ACCELERATION_DUE_TO_GRAVITY * 0.06)
...
def apply_steer_angle_limits_vm(apply_angle: float, apply_angle_last: float, v_ego_raw: float, steering_angle: float,
  ...
  """Apply vehicle-model lateral jerk/acceleration and EPS angle limits."""
  v_ego_raw = max(v_ego_raw, 1.0)

  max_curvature_rate_sec = limits.ANGLE_LIMITS.MAX_LATERAL_JERK / (v_ego_raw ** 2)
  max_angle_rate_sec = math.degrees(VM.get_steer_from_curvature(max_curvature_rate_sec, v_ego_raw, 0.0))
  max_angle_delta = min(max_angle_rate_sec * (DT_CTRL * limits.STEER_STEP),
                        limits.ANGLE_LIMITS.MAX_ANGLE_RATE)
  new_apply_angle = rate_limit(apply_angle, apply_angle_last, -max_angle_delta, max_angle_delta)

  max_curvature = limits.ANGLE_LIMITS.MAX_LATERAL_ACCEL / (v_ego_raw ** 2)
  max_angle = math.degrees(VM.get_steer_from_curvature(max_curvature, v_ego_raw, 0.0))
  new_apply_angle = float(np.clip(new_apply_angle, -max_angle, max_angle))

  if not lat_active:
    new_apply_angle = steering_angle
  return float(np.clip(new_apply_angle, -limits.ANGLE_LIMITS.STEER_ANGLE_MAX,
